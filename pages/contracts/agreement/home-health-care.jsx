...
const HomeHealthCare = ({ data }) => {
  const { user } = useContext(AuthContext);
  const signRef = useRef();
  const vendorSignRef = useRef();
  const vendorSignRefOne = useRef();
  ...
  useEffect(() => {
    ...
    const date = new Date(data.MoUStartDate * 1000);
    setDay(date.getDate());
    setMonth(monthNames[date.getMonth()]);
    setYear(date.getFullYear());
    setServiceProvider(data.BusinessInfo.PartnerName);
    setRegisteredOffice(
      data.BusinessInfo.StreetAddress +
        ' ' +
        data.BusinessInfo.LandMark +
        ' ' +
        data.BusinessInfo.City +
        ' ' +
        data.BusinessInfo.State +
        '  - ' +
        data.BusinessInfo.PinCode,
    );
    setSigningAuthority(data.BusinessInfo.ContactName);
    setLocation(data.BusinessInfo.City);
    setPhoneNo(data.BusinessInfo.ContactNo);
    setEmail(data.BusinessInfo.EmailID);
    setPoc(data.BusinessInfo.ContactName);
    setRateCards(data.ContractDetails);
    //setSignature(cdata.signature);
    // if (signature) {
    //   vendorSignRef.current.fromDataURL(signature);
    //   vendorSignRef.current.off();
    //   vendorSignRefOne.current.fromDataURL(signature);
    //   vendorSignRefOne.current.off();
    // }
  }, [data]);
  ...
  return (
    <section style={{ padding: '0px' }}>
      ...
      <p>
        ...
        [Signing Authority <b>{signingAuthority} </b> - Manager Operations ];
      </p>
      <p>
      ...
      >
        ...
        <li>
          ...
          sale, labs and diagnostics, medicines and ambulance in {location}.
        </li>
        <li>
      ...
      <ol>
        ...
        <li>
          ...
          and Information of <u>Anvayaa</u> on its website and other
          communication and offer services on behalf of Anvayaa without any
          breach of intellectual property rights. The brand logo / registered
          trademark of Anvayaa and Service Provider would remain the sole
          property of respective party and neither Anvayaa nor Service Provider
          will have no rights over it whatsoever on other party, except for
          usage on website and for marketing purposes.
        </li>
        <li>
          Service Provider agrees that it shall be solely responsible and liable
          for the consequences of any fault or negligence of any of its
      ...
      >
        <div>
          ...
          />
          <p>
            Rep by: <b>Prashanth Reddy</b> <br /> DIRECTOR
          </p>
        </div>
        <div>
          <p>M/s {serviceProvider}</p>
          <br />
          <br />
          <SignatureCanvas
            ref={vendorSignRef}
      ...
      >
        <h1>Rate Card</h1>
        <h2>M/s {serviceProvider}</h2>
        {rateCards &&
          rateCards.length > 0 &&
          rateCards[0]?.RateCard?.[0]?.ReferalType == 'Referral_Percentage' && (
            <h5>
              Following is agreed rate card for each of the services with an
              Anavyaa referral percentage of{' '}
              {rateCards[0]?.RateCard?.[0]?.ReferalFee}%. The vendor should
              invoice in Anvayaa as per final rates mentioned in the rate card
              provided below.
            </h5>
          )}
        {rateCards &&
          rateCards.length > 0 &&
          rateCards[0]?.RateCard?.[0]?.ReferalType == 'Referral_Amount' && (
            <h5>
              Following is agreed rate card for each of the services with an
        ...
        <p>
          All rates mentioned above are agreed upon and are subject to the terms
          and conditions outlined in the contract. Any changes to the rates will
          be mutually agreed upon in writing by both parties.
        </p>
        <p>
          All the payments will be collected by Anvayaa. There will be monthly
          settlement for all the payments.
          <br />
          The payments will be released on the following month between 7 to 10
          working day after bills are verified.
        </p>
...
HomeHealthCare.getLayout = function getLayout(page) {
