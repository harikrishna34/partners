...
const JoinUs = () => {
  ...
  const login = (phoneNo) => {
    axios
      .post(Rest.login, { ContactNo: phoneNo, Type: 'Joinus' })
      .then((res) => {
        const data = res.data;
  ...
  const verifyOtp = async () => {
    ...
    //bodyParams.OTPCode = loginData.data.OTP;
          ...
          <form onSubmit={form.onSubmit(handleLogin)}>
            <SimpleGrid cols={1} verticalSpacing="lg">
              <TextInput
                ...
                label="Phone No"
                description="Enter a valid phone number you have access to, we would verify this number. Standard Rates Apply."
                withAsterisk
                size={size}
                {...form.getInputProps('phoneNo')}
              />
              <Button id="register-button" size={size} type="submit">
                Register
              </Button>
              <Button
...
JoinUs.getLayout = function getLayout(page) {
